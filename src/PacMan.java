...
public class PacMan extends JPanel implements ActionListener, KeyListener {
    ...
    private void showInstructions() {
        String message = "🎮 Welcome to Pac-Man!\n\n" +
                ...
                "🍒 Eat all the white dots to gain points\n" +
                "👻 Avoid the ghosts or you'll lose lives\n" +
                "🌀 Teleport through the tunnel (row 9)\n" +
                "❤️ You have 3 lives. Good luck!\n\n" +
    ...
    public void draw(Graphics g) {
        ...
        g.drawString(highScoreText, (boardWidth - textWidth) / 2, tileSize / 2);
        // Left aligned info
        if (gameOver) {
            g.drawString("Game Over- Score: " + score, tileSize / 2, tileSize / 2);
        } else {
            g.drawString("x" + lives + "Score: " + String.valueOf(score), tileSize / 2, tileSize / 2);
        }
    ...
    public void move() {
        ...
        // Teleport Pac-Man through tunnel
        if (pacman.y == 9 * tileSize) {
            if (pacman.x + pacman.width <= 0) { // left edge
                pacman.x = boardWidth - tileSize;
            } else if (pacman.x >= boardWidth) { // right edge
                pacman.x = 0;
            }
        }
        ...
        for (Block ghost : ghosts) {
            ...
            // Teleport ghosts through tunnel
